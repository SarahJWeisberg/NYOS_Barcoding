...
#Change "Atlantic Butterfish" and "Gulf Butterfish" to "Butterfish"
# fish_filter<-fish_filter%>%
#   mutate(Common_Name = replace(Common_Name,Common_Name == "Atlantic Butterfish" | Common_Name =="Gulf Butterfish", "Butterfish"))

# #separate by cruise
# spring_21<-filter(fish_filter,Cruise== 2105)
# 
# #separate eggs out
# spring_21_eggs<-filter(spring_21, Stage == "Egg") %>% select(-"Stage")
# 
# #summarize eggs by station
# spring_21_eggs<-count(spring_21_eggs, Waypoint, Common_Name) 
# 
# #separate larvae out
# spring_21_larvae<-filter(spring_21, Stage == "Larva")  %>% select(-"Stage")
# 
# #summarize larvae by station
# spring_21_larvae<-count(spring_21_larvae, Waypoint, Common_Name)

#think there's a more efficient tidy way to do this
...
#load in all data, change Waypoint column to match above
# all_stations<-read_csv(file = "NYOS2105_all.csv")
# colnames(all_stations)[1]<-"Waypoint"
# all_stations$Waypoint<-as.character(all_stations$Waypoint)
# 
# #remove 7.11 (extra sampling) - for now
# all_stations<-all_stations[-40,]
# #remove notes columns
# all_stations<-all_stations[,c(1,3,4)]
...
#add in columns for eggs barcoded
#remove NAs
# eggs_map <- left_join(study_area,spring_21_eggs, by = "Waypoint")
# #eggs_map[is.na(eggs_map)]<-0
# eggs_map<-eggs_map[complete.cases(eggs_map),]
# 
# #add in columns for larvae barcoded
# #remove NAs
# larvae_map <- left_join(study_area,larvae, by = "Waypoint")
# #eggs_map[is.na(eggs_map)]<-0
# larvae_map<-larvae_map[complete.cases(larvae_map),]

# #add in columns for all eggs & larvae found
# all_map<-left_join(study_area,stations,by="Waypoint")
# all_map[all_map == 0] <-NA
