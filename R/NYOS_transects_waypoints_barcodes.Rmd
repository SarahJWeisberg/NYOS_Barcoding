...
cruises<-unique(station_data$Cruise_ID)

station_data_clean <- station_data %>% 
  select(Cruise_ID,Station_ID,Latitude,Longitude) %>%
  mutate(season = substring(Cruise_ID,4)) %>%
  group_by(Station_ID) %>%
  mutate(mean_lat = mean(Latitude), mean_lon = mean(Longitude)) 

station_locations<- station_data_clean %>% select(Station_ID,mean_lat,mean_lon)
...
station_data_summary <- left_join(station_data_summary,station_locations, by = "Station_ID") 
station_data_summary <- station_data_summary %>%
  mutate(mean_lon = -(mean_lon))
...
s<- basemap +
   geom_scatterpie(data= station_data_summary, 
                   aes(x=mean_lon, y=mean_lat, r =0.02*total),
                   cols=c("Spring","Summer","Fall","Winter"),linewidth=0.1,
                   legend_name = "Season")
s
ggplot()+
   geom_scatterpie(data= station_data_summary, 
                   aes(x=mean_lon, y=mean_lat, r =0.015*total),cols=c("5","2","7","9"),color=NA,alpha=0.8)
basemap
